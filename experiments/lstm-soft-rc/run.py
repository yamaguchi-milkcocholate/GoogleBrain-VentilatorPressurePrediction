...
def main(config: Dict[str, Any]):
    ...
    train_df = reduce_mem_usage(pd.read_csv(cachedir / f"train-10fold-cut-only-soft-rc-debug{config.debug}.csv"))
    test_df = reduce_mem_usage(pd.read_csv(cachedir / f"test-10fold-cut-only-soft-rc-debug{config.debug}.csv"))
    ...
    cont_features = [f for f in features if ("RC_" not in f) and ("u_out" not in f)]
