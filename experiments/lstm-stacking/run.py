...
def main(config: Dict[str, Any]):
    ...
    for savedir in config.savedirs:
        sd = basedir / ".." / ".." / "logs" / savedir
        train_pred = pd.read_csv(sd / "valid_preds.csv", index_col=0).values.reshape(-1)
        test_pred = pd.read_csv(sd / "submission.csv").pressure.values
        train_df[savedir] = train_pred[: train_df.shape[0]]
        test_df[savedir] = test_pred[: test_df.shape[0]]
