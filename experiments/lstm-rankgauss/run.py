...
def main(config: Dict[str, Any]):
    ...
    # MMS = MinMaxScaler()
    # train_df[cont_features] = MMS.fit_transform(train_df[cont_features])
    # test_df[cont_features] = MMS.transform(test_df[cont_features])
    # PT = PowerTransformer()
    # train_df[cont_features] = PT.fit_transform(train_df[cont_features])
    # test_df[cont_features] = PT.transform(test_df[cont_features])

    QT = QuantileTransformer()
    train_df[cont_features] = QT.fit_transform(train_df[cont_features])
    test_df[cont_features] = QT.transform(test_df[cont_features])
    MMS = MinMaxScaler()
    train_df[cont_features] = MMS.fit_transform(train_df[cont_features])
    test_df[cont_features] = MMS.transform(test_df[cont_features])
